...
BITGET_SYMBOL = os.getenv("BITGET_E2E_SYMBOL", "BTCUSDT").upper()
BITGET_MIN_USDT_BALANCE = float(os.getenv("BITGET_E2E_MIN_USDT_BALANCE", "5"))
BITGET_SPOT_MIN_USDT_BALANCE = float(os.getenv("BITGET_E2E_SPOT_MIN_USDT_BALANCE", "5"))
BITGET_SPOT_RISK_PER_TRADE_PERCENT = float(
    os.getenv("BITGET_E2E_SPOT_RISK_PER_TRADE_PERCENT", "2.0")
)
BITGET_SPOT_MAX_POSITION_PCT_BALANCE = float(
    os.getenv("BITGET_E2E_SPOT_MAX_POSITION_PCT_BALANCE", "0.05")
)
BITGET_CCXT_TIMEOUT_MS = int(os.getenv("BITGET_E2E_CCXT_TIMEOUT_MS", "8000"))
...
def _bitget_credentials():
    api_key = os.getenv("TESTNET_BITGET_API_KEY")
    api_secret = os.getenv("TESTNET_BITGET_API_SECRET")
    password = os.getenv("TESTNET_BITGET_PASSWORD")
    if not api_key or not api_secret or not password:
        pytest.skip(
            "Bitget testnet keys or password are not set. Expected TESTNET_BITGET_API_KEY, "
            "TESTNET_BITGET_API_SECRET, and TESTNET_BITGET_PASSWORD."
        )
...
async def _cleanup_bitget_symbol(executor, symbol: str) -> None:
...
@pytest_asyncio.fixture(params=BITGET_CASES)
async def bitget_controller_case(request, monkeypatch):
    market_type = request.param["market_type"]
    direction = request.param["direction"]
    api_key, api_secret = _bitget_credentials()
    ...
    risk_per_trade_percent = (
        BITGET_SPOT_RISK_PER_TRADE_PERCENT if market_type == "spot" else 5.0
    )
    max_position_pct_balance = (
        BITGET_SPOT_MAX_POSITION_PCT_BALANCE if market_type == "spot" else 0.5
    )
    monkeypatch.setattr(
    ...
    executor._exchange.timeout = BITGET_CCXT_TIMEOUT_MS
    if executor._exchange_pro:
        executor._exchange_pro.timeout = BITGET_CCXT_TIMEOUT_MS

    try:
        await asyncio.wait_for(
            executor._exchange.fetch_time(),
            timeout=max(5.0, BITGET_CCXT_TIMEOUT_MS / 1000.0 + 2.0),
        )
    except Exception as exc:
        await executor.close()
        if not session.closed:
            await session.close()
        pytest.skip(f"Bitget testnet is not reachable from this environment: {exc}")

    await _cleanup_bitget_symbol(executor, BITGET_SYMBOL)
    ...
    strategy_config_id = f"bitget-e2e-{market_type}-{direction.name.lower()}"
...
async def _wait_for_open_position(controller: TradingController, timeout: float = 45.0):
    ...
    while time.monotonic() < deadline:
        ...
        await asyncio.sleep(0.5)
    async with controller._positions_dict_lock:
        return controller._active_position_get(BITGET_SYMBOL)
...
async def _force_entry_fill_if_websocket_did_not_deliver(
    ...
    ccxt_symbol = executor._normalize_symbol(BITGET_SYMBOL)
    deadline = time.monotonic() + timeout
    while time.monotonic() < deadline:
        mapped_order = None
        try:
        ...
        if getattr(executor, "market_type", None) == "spot":
            ...
            base_asset = (
                BITGET_SYMBOL[:-4] if BITGET_SYMBOL.endswith("USDT") else BITGET_SYMBOL
            )
            free_base = float(
    ...
    if getattr(executor, "market_type", None) == "spot":
        ...
        if free_base >= entry_qty * 0.9:
            ...
            try:
                ...
                pass
            pytest.fail(
                f"Bitget spot entry order {entry_order_id} was not filled by testnet; "
...
async def _assert_bitget_exit_and_dca_orders(
    ...
    assert not missing_ids, (
        f"Bitget {market_type} did not expose all controller-tracked orders as open. "
        f"Missing IDs: {missing_ids}; open={open_orders}; algo={algo_orders}"
    )


async def _assert_bitget_spot_sl_and_dca_orders(controller: TradingController) -> None:
    ...
    missing_ids = expected_ids - all_open_order_ids
    assert not missing_ids, (
        f"Bitget spot did not expose SL/DCA orders as open. "
        f"Missing IDs: {missing_ids}; open={open_orders}; algo={algo_orders}"
    )
...
async def _place_and_assert_spot_partial_tps_sequentially(
    ...
    await controller._cancel_all_exit_orders(
        BITGET_SYMBOL, "BITGET_E2E_SPOT_SL_CHECKED"
    )
    ...
    for idx in range(2):
        ...
        assert placed, f"Bitget spot partial TP #{idx + 1} was not placed sequentially."

    open_orders = await executor.get_open_orders(BITGET_SYMBOL)
    ...
    assert expected_tp_ids <= open_order_ids, (
        f"Bitget spot did not expose sequential partial TPs as open. "
        f"Missing IDs: {expected_tp_ids - open_order_ids}; open={open_orders}"
    )

    await controller._cancel_all_exit_orders(
        BITGET_SYMBOL, "BITGET_E2E_SPOT_TP_CHECKED"
    )
    tp_cancelled = await _wait_until(
        lambda: _position_without_open_tps(controller),
        timeout=20.0,
    )
    assert tp_cancelled, "Bitget spot partial TPs were not cleared before final close."
...
@pytest.mark.e2e
@pytest.mark.asyncio
async def test_bitget_controller_signal_to_position_sl_tp_dca_and_close(
    bitget_controller_case,
):
    controller: TradingController = bitget_controller_case["controller"]
    executor = bitget_controller_case["executor"]
    market_type = bitget_controller_case["market_type"]
    direction = bitget_controller_case["direction"]
    strategy_config_id = bitget_controller_case["strategy_config_id"]

    if market_type == "spot" and direction == SignalDirection.SHORT:
        pytest.skip(
            "Bitget spot does not support short positions through this controller path."
        )

    balances = await executor.get_account_balance()
    print(f"\n[DEBUG E2E] market_type={market_type} balances={balances}")
    if balances is None:
        pytest.skip(
            "Could not fetch Bitget account balance; testnet/private API is unavailable."
        )

    usdt_free = float((balances.get("USDT") or {}).get("free", 0) or 0)
    required_usdt_balance = (
        BITGET_MIN_USDT_BALANCE
        if market_type == "futures_usdtm"
        else BITGET_SPOT_MIN_USDT_BALANCE
    )
    print(f"[DEBUG E2E] usdt_free={usdt_free}, required={required_usdt_balance}")

    if usdt_free < required_usdt_balance:
        # Try fetching full raw balance as well
        try:
            raw_bal = await executor._exchange.fetch_balance()
            print(f"[DEBUG E2E] Raw balance without params: {raw_bal.get('USDT')}")
            raw_swap_bal = await executor._exchange.fetch_balance({"type": "swap"})
            print(f"[DEBUG E2E] Raw balance with type=swap: {raw_swap_bal.get('USDT')}")
        except Exception as e:
            print(f"[DEBUG E2E] Error fetching raw balances: {e}")

        pytest.skip(
            f"Insufficient Bitget testnet USDT balance for {market_type}: "
            f"{usdt_free} < {required_usdt_balance}. Please transfer USDT from spot to futures "
            f"on your Bitget demo account."
        )

    # Force-clear any leftover trigger/plan orders from previous failed test runs
    try:
        ccxt_symbol = BITGET_SYMBOL.replace("USDT", "/USDT")
        leftover_orders = await executor._exchange.fetch_open_orders(
            ccxt_symbol, params={"stop": True}
        )
        if isinstance(leftover_orders, list):
            for lo in leftover_orders:
                try:
                    lo_id = lo.get("id")
                    if lo_id:
                        lo_info = lo.get("info") or {}
                        lo_params = {"stop": True, "trigger": True}
                        if "planType" in lo_info:
                            lo_params["planType"] = lo_info["planType"]
                        await executor._exchange.cancel_order(
                            lo_id, ccxt_symbol, lo_params
                        )
                except Exception:
                    pass
    except Exception:
    ...
    await executor.cancel_all_open_orders(BITGET_SYMBOL)
    ...
    try:
        pair_info = {
            ...
            "tick_size": 0.01
            if market_type == "spot"
            else (await executor.get_tick_size(BITGET_SYMBOL) or 0.01),
            "lot_params": await executor.get_lot_size_params(BITGET_SYMBOL)
            or {"minQty": 0.00001, "maxQty": 100.0, "stepSize": 0.00001},
            "min_notional": await executor.get_min_notional(BITGET_SYMBOL) or 1.0,
        ...
        controller.consumer.update_pair_data(BITGET_SYMBOL, pair_info)
        ...
        assert position is not None and position.status == "OPEN", (
            f"Bitget {market_type}/{direction.name} position was not opened. "
        ...
        if market_type == "spot":
            await _assert_bitget_spot_sl_and_dca_orders(controller)
            await _place_and_assert_spot_partial_tps_sequentially(controller, executor)
        else:
            await _assert_bitget_exit_and_dca_orders(controller, market_type)

        await controller.close_position(
            BITGET_SYMBOL, reason="BITGET_E2E_FULL_PATH_CLOSE"
        )
        ...
        for _ in range(15):
            open_orders_after_close = await executor.get_open_orders(BITGET_SYMBOL)
            if not open_orders_after_close:
                break
            # Force cancel any lingering trigger/plan orders manually
            try:
                ccxt_symbol = BITGET_SYMBOL.replace("USDT", "/USDT")
                for o_rem in open_orders_after_close:
                    o_rem_id = o_rem.get("id") or o_rem.get("orderId")
                    if o_rem_id:
                        await executor._exchange.cancel_order(
                            o_rem_id, ccxt_symbol, {"stop": True, "trigger": True}
                        )
            except Exception:
                pass
        ...
        assert not open_orders_after_close, (
            f"Bitget {market_type}/{direction.name} left open orders after close: "
            f"{open_orders_after_close}"
        )
    except Exception as e:
        print("CRITICAL ERROR IN MAIN E2E LOGIC:")
        traceback.print_exc()
        raise e
...
def _position_removed(controller: TradingController):
    async def check():
        async with controller._positions_dict_lock:
            return controller._active_position_get(BITGET_SYMBOL) is None
